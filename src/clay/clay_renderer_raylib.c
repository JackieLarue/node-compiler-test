        ...
        switch (renderCommand->commandType) {
            case CLAY_RENDER_COMMAND_TYPE_TEXT: {
                ...
                DrawTextEx(
                    ...
                    CLAY_COLOR_TO_RAYLIB_COLOR(renderCommand->config.textElementConfig->textColor)
                );
                free(cloned);
                break;
            }
            ...
            case CLAY_RENDER_COMMAND_TYPE_BORDER: {
                ...
                if (config->right.width > 0) {
                    DrawRectangle(
                        (int)roundf(boundingBox.x + boundingBox.width - config->right.width),
                ...
                if (config->bottom.width > 0) {
                    DrawRectangle(
                        ...
                        (int)roundf(boundingBox.y + boundingBox.height - config->bottom.width),
                        (int)roundf(
            ...
            case CLAY_RENDER_COMMAND_TYPE_CUSTOM: {
                CustomLayoutElement* customElement =
                    (CustomLayoutElement*)renderCommand->config.customElementConfig->customData;
                if (!customElement)
                    continue;
